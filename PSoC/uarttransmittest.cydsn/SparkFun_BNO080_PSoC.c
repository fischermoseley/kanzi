...
#include "SparkFun_BNO080_PSoC.h"

//Attempt communication with the device
//Return true if we got a 'Polo' back from Marco
boolean BNO080::begin(uint8_t deviceAddress, uint8_t intPin)
{
...
void BNO080::parseInputReport(void)
{
	...
	else if (shtpData[5] == SHTP_REPORT_COMMAND_RESPONSE)
	{
		...
		if (command == COMMAND_ME_CALIBRATE)
		{
			HOST_UART_PutString("ME Cal report found!");
...
bool BNO080::readFRSdata(uint16_t recordID, uint8_t startLocation, uint8_t wordsToRead)
{
	...
	while (1)
	{
		...
		{
			if (_printDebug == true)
				HOST_UART_PutString("metaData array over run. Returning.");
...
boolean BNO080::receivePacket(void)
{
	...
	uint8_t packetLSB = IMU_I2C_MasterReadByte();
	uint8_t packetMSB = IMU_I2C_MasterReadByte();
	uint8_t channelNumber = IMU_I2C_MasterReadByte();
	uint8_t sequenceNumber = IMU_I2C_MasterReadByte(); //Not sure if we need to store this or not
...
boolean BNO080::getData(uint16_t bytesRemaining)
{
	...
	while (bytesRemaining > 0)
	{
		...
		//The first four bytes are header bytes and are thrown away
		IMU_I2C_MasterReadByte();
		IMU_I2C_MasterReadByte();
		IMU_I2C_MasterReadByte();
		IMU_I2C_MasterReadByte();		
		...
		{
			uint8_t incoming = IMU_I2C_MasterReadByte();
...
boolean BNO080::sendPacket(uint8_t channelNumber, uint8_t dataLength)
{
	...
	IMU_I2C_MasterSendStart(_deviceAddress, 0); //send to _deviceAddress, but in write mode (hence the 0)

	//Send the 4 byte packet header
	IMU_I2C_MasterReadByte(packetLength & 0xFF);			  //Packet length LSB
	IMU_I2C_MasterReadByte(packetLength >> 8);				  //Packet length MSB
	IMU_I2C_MasterReadByte(channelNumber);					  //Channel number
	IMU_I2C_MasterReadByte(sequenceNumber[channelNumber]++); //Send the sequence number, increments with each packet sent, different counter for each channel

	//Send the user's data packet
	for (uint8_t i = 0; i < dataLength; i++)
	{
		IMU_I2C_MasterReadByte(shtpData[i]);
	}
	if (IMU_I2C_MasterSendStop() != I2C_MSTR_NO_ERROR)
	{
		return (false);
	}
	return (true);
...
void BNO080::printPacket(void)
{
	if (_printDebug == true)
	{
		...
		HOST_UART_PutString("Header:");
		for (uint8_t x = 0; x < 4; x++)
		{
			HOST_UART_PutString(" ");
			if (shtpHeader[x] < 0x10)
				HOST_UART_PutString("0");
		...
		{
			HOST_UART_PutString(" [Continued packet] ");
			packetLength &= ~(1 << 15);
		}

		HOST_UART_PutString(" Length:"));
		_debugPort->print(packetLength);

		HOST_UART_PutString(" Channel:");
		if (shtpHeader[2] == 0)
			HOST_UART_PutString("Command");
		else if (shtpHeader[2] == 1)
			HOST_UART_PutString("Executable");
		else if (shtpHeader[2] == 2)
			HOST_UART_PutString("Control");
		else if (shtpHeader[2] == 3)
			HOST_UART_PutString("Sensor-report");
		else if (shtpHeader[2] == 4)
			HOST_UART_PutString("Wake-report");
		else if (shtpHeader[2] == 5)
			HOST_UART_PutString("Gyro-vector");
		else
