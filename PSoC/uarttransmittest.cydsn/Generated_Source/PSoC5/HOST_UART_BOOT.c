...
#if defined(CYDEV_BOOTLOADER_IO_COMP) && (0u != ((CYDEV_BOOTLOADER_IO_COMP == CyBtldr_HOST_UART) || \
         ...
         
{
    ...
    while(bufIndex < size)
    {
        HOST_UART_PutChar(pData[bufIndex]);
        bufIndex++;
    }
         ...
         
{
    ...
    *  If at least one byte is received within the time out interval, wait for more data */
    for (iCntr = 0u; iCntr < ((uint16)10u * timeOut); iCntr++)
    {
        ...
        */
        if(0u != HOST_UART_GetRxBufferSize())
        {
            ...
            */
            do
            {
                oldDataCount = HOST_UART_GetRxBufferSize();
                CyDelay(HOST_UART_BYTE2BYTE_TIME_OUT);
            }
            while(HOST_UART_GetRxBufferSize() > oldDataCount);

            status = CYRET_SUCCESS;
            break;
        }
        /* If the data is not received, give a delay of 
        *  HOST_UART_BL_CHK_DELAY_MS and check again until the timeOut specified.
        */
        else
        {
            CyDelay(HOST_UART_BL_CHK_DELAY_MS);
        }
    }
    ...
    *count = 0u;
    ...
    while(HOST_UART_GetRxBufferSize() > 0u)
    {
        tempCount = HOST_UART_GetRxBufferSize();
        *count  =(*count) + tempCount;
        ...
        if(*count < size)
        {
            for (iCntr = 0u; iCntr < tempCount; iCntr++)
            {
                ...
                pData[dataIndexCntr] = HOST_UART_ReadRxData();
                dataIndexCntr++;
            }

            /* Check if the last received byte is end of packet defined by bootloader
            *  If not wait for additional HOST_UART_WAIT_EOP_DELAY ms.
            */
            if(pData[dataIndexCntr - 1u] != HOST_UART_PACKET_EOP)
            {
                CyDelay(HOST_UART_WAIT_EOP_DELAY);
            }
        }
