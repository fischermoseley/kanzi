...
/*******************************************************************************
* Function Name: IMU_UART_IntClock_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void IMU_UART_IntClock_Start(void) 
{
    /* Set the bit to enable the clock. */
    IMU_UART_IntClock_CLKEN |= IMU_UART_IntClock_CLKEN_MASK;
	IMU_UART_IntClock_CLKSTBY |= IMU_UART_IntClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: IMU_UART_IntClock_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void IMU_UART_IntClock_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    IMU_UART_IntClock_CLKEN &= (uint8)(~IMU_UART_IntClock_CLKEN_MASK);
	IMU_UART_IntClock_CLKSTBY &= (uint8)(~IMU_UART_IntClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: IMU_UART_IntClock_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void IMU_UART_IntClock_StopBlock(void) 
{
    if ((IMU_UART_IntClock_CLKEN & IMU_UART_IntClock_CLKEN_MASK) != 0u)
    {
...
#if defined(IMU_UART_IntClock__CFG3)
        CLK_DIST_AMASK = IMU_UART_IntClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = IMU_UART_IntClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* IMU_UART_IntClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(IMU_UART_IntClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(IMU_UART_IntClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: IMU_UART_IntClock_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void IMU_UART_IntClock_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        IMU_UART_IntClock_CLKSTBY &= (uint8)(~IMU_UART_IntClock_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: IMU_UART_IntClock_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void IMU_UART_IntClock_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = IMU_UART_IntClock_GetSourceRegister();
    uint16 oldDivider = IMU_UART_IntClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = IMU_UART_IntClock_CLKEN & IMU_UART_IntClock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(IMU_UART_IntClock_DIV_PTR, clkDivider);
                IMU_UART_IntClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                IMU_UART_IntClock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(IMU_UART_IntClock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((IMU_UART_IntClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(IMU_UART_IntClock_DIV_PTR, clkDivider);
				IMU_UART_IntClock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: IMU_UART_IntClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 IMU_UART_IntClock_GetDividerRegister(void) 
{
    return CY_GET_REG16(IMU_UART_IntClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: IMU_UART_IntClock_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void IMU_UART_IntClock_SetModeRegister(uint8 modeBitMask) 
{
    IMU_UART_IntClock_MOD_SRC |= modeBitMask & (uint8)IMU_UART_IntClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: IMU_UART_IntClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void IMU_UART_IntClock_ClearModeRegister(uint8 modeBitMask) 
{
    IMU_UART_IntClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(IMU_UART_IntClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: IMU_UART_IntClock_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 IMU_UART_IntClock_GetModeRegister(void) 
{
    return IMU_UART_IntClock_MOD_SRC & (uint8)(IMU_UART_IntClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: IMU_UART_IntClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void IMU_UART_IntClock_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = IMU_UART_IntClock_GetDividerRegister();
    uint8 oldSrc = IMU_UART_IntClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        IMU_UART_IntClock_MOD_SRC |= CYCLK_SSS;
        IMU_UART_IntClock_MOD_SRC =
            (IMU_UART_IntClock_MOD_SRC & (uint8)(~IMU_UART_IntClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: IMU_UART_IntClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 IMU_UART_IntClock_GetSourceRegister(void) 
{
    return IMU_UART_IntClock_MOD_SRC & IMU_UART_IntClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: IMU_UART_IntClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void IMU_UART_IntClock_SetPhaseRegister(uint8 clkPhase) 
{
    IMU_UART_IntClock_PHASE = clkPhase & IMU_UART_IntClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: IMU_UART_IntClock_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 IMU_UART_IntClock_GetPhaseRegister(void) 
{
    return IMU_UART_IntClock_PHASE & IMU_UART_IntClock_PHASE_MASK;
}
